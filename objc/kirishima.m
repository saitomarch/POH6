...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        NSString *(^readData)() = ^{
            return [[NSString alloc] initWithData:[[NSFileHandle fileHandleWithStandardInput] readDataToEndOfFile] encoding:NSUTF8StringEncoding];
        };
        ...
        NSInteger numOfCells = [lines[offset++] integerValue];
        NSInteger goal = numOfCells - 1;

        NSArray *cells = [lines[offset++] componentsSeparatedByString:@" "];

        NSInteger numOfActs = [lines[offset++] integerValue];
        ...
        for (NSInteger count = 0; count < numOfActs; count++) {
            NSInteger actNum = [lines[count + offset] integerValue];
            ...
            while (status == CURRENT) {
                if (actNum == goal) {
                    ...
                    NSInteger moves = [cells[actNum] integerValue];
                    if (moves == 0) {
                        status = DEAD;
                    }else{
                        actNum += moves;
