...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSInteger times = [lines[0] integerValue];
        for (int count = 1; count <= times; count++) {
            NSArray *actArr = [lines[count] componentsSeparatedByString:@" "];
            NSInteger action = [actArr[0] integerValue];
            float quantity = [actArr[1] floatValue];
            switch (action) {
                case 1:
